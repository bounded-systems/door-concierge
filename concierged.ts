...
// The published concierge-wire agreement, bundled next to concierged in the
// image. When absent (running the source in tests) it's null and the check is
// skipped. It NEVER rejects: a mismatch is LOGGED (surfacing spec↔handler
// drift), request handling is unaffected.
...
let wireManifest: WireManifest | null = (() => {
  try {
    const p = new URL("./concierge-wire.manifest.json", import.meta.url).pathname;
...
export function shadowCheckParams(
  ...
  if (unexpected.length) {
    console.warn(
      `concierge-wire: request "${method}" sends undeclared param(s): ${
        unexpected.join(", ")
      } — spec/handler drift`,
    );
  }
